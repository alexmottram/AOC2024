...
namespace solutions {
    ...
    public:
        ...
        RoundScore(const std::string& str)
                :red(0), blue(0), green(0)
        {
            auto split_str = utils::split_string(str, ",", true);
            for (auto colour_str : split_str) {
        ...
        bool score_valid(const int& max_red, const int& max_blue, const int& max_green)
        {
    ...
    long long day_2_part_a(const utils::InputReader& input)
    {
        ...
        for (const auto game : games) {
            ...
            for (auto round : rounds) {
                ...
                bool valid_score = round_score.score_valid(max_red, max_blue,
                        max_green);
    ...
    long long day_2_part_b(const utils::InputReader& input)
    {
        ...
        for (const auto game : games) {
            ...
            std::cout << "Score for game " << game_num << " --> " << game_score
                      << std::endl;
