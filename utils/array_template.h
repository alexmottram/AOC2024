...
namespace utils {
	...
	public:
		...
		using RefNodeType = RefNode<size_type, value_type>;
		using ConstNodeType = ConstNode<size_type, value_type>;
		...
		template<typename x_size_type, typename y_size_type>
		RefNodeType node_at(const x_size_type &x, const y_size_type &y) {
			reference val = at(x, y);
			return RefNode{val, static_cast<size_t>(x), static_cast<size_t>(y)};
		}

		template<typename x_size_type, typename y_size_type>
		ConstNodeType const_node_at(const x_size_type &x, const y_size_type &y) const {
			auto val = at(x, y);
			ConstNodeType this_node{val, static_cast<size_t>(x), static_cast<size_t>(y)};
			return this_node;
		}
		...
		std::set<RefNodeType> find(const_reference val) {
			std::set<RefNodeType> found_nodes;
